...
class Snake(Problem):
    ...
    def h(self,node):
        snake_x,snake_y= node.state[0][-1]
        greens = node.state[2]
        if len(greens)==0:
            return 0
        distance = 1000
        for x, y in greens:
            distance = min(abs(snake_x - x) + abs(snake_y - y), distance)

        return distance + len(greens) - 1


if __name__ == '__main__':
    n = int(input())
    zeleni = [tuple(map(int, input().split(','))) for _ in range(n)]
