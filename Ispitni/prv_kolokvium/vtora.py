    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            for j in range(len(state)):
                stolbovi = [list(e) for e in state]

                if i==j or stolbovi[i][0] == 1000: continue

                if stolbovi[i][-1] <= stolbovi[j][-1]:
                    transfer = stolbovi[i].pop()
                    if len(stolbovi[i])==0: stolbovi[i].append(1000)

                    stolbovi[j].append(transfer)
                    stolbovi[j] = [d for d in stolbovi[j] if d!=1000]
                    stolbovi = [tuple(e) for e in stolbovi]
                    successors[f'MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}'] = tuple(stolbovi)
...
if __name__ == "__main__":
    ...
    n = line1.split(";")
    list_initial = line1.split(";")
    list_goal = line2.split(";")

    initial_state = []

    for l in list_initial:
        tmp_l = l.split(",")
        tmp_l = [1000 if len(l) == 0 else int(l1) for l1 in tmp_l]

        initial_state.append(tuple(tmp_l))
    ...
    for l in list_goal:
        tmp_l = l.split(",")
        tmp_l = [1000 if len(l) == 0 else int(l1) for l1 in tmp_l]
        goal_state.append(tuple(tmp_l))

    initial_state = tuple(initial_state) # ( (3,2,1), (1000,) , (1000,))
    goal_state = tuple(goal_state)

    d= DonutGame(initial_state,goal_state)
    solution = breadth_first_graph_search(d)
    if solution:
        print(f"Number of action {len(solution.solution())}")
