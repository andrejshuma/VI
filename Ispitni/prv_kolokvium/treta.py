...
class Squares(Problem):
    ...
    def successor(self, state):
        succ = {}
        # state=((x1,y1),(x2,y2),(x3,y3),(x4,y4),(x5,y5))
        squares = list(state)
        moves = ["gore", "dolu", "levo", "desno"]
        directions = [(0, 1), (0, -1), (-1, 0), (+1, 0)]

        for square_i in range(0, len(squares)):
            new_state_part1 = squares[0:square_i]
            for move, coord in zip(moves, directions):
                new_square = (squares[square_i][0] + coord[0], squares[square_i][1] + coord[1])
                new_state = new_state_part1 + [new_square] + squares[square_i + 1:]
                if self.check_valid(new_state):
                    # print(new_state)
                    succ[f"Pomesti kvadratche {square_i + 1} {move}"] = tuple(new_state)
