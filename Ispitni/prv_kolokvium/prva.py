...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        if state in walls: return False
        return 0<=state[0]<n and 0<=state[1]<n

    def successor(self, state):
        successors = dict()

        moves = ["Gore 1","Gore 2","Gore 3","Desno 1","Desno 2","Desno 3"]
        moves_cords = [ (0,1),(0,2),(0,3),(1,0),(2,0),(3,0)]

        for move,cord in zip(moves,moves_cords):
            new_ghost_pos = (state[0] + cord[0],state[1]+cord[1])
            if self.check_valid(new_ghost_pos,self.walls,self.n):
                successors[move] = new_ghost_pos
