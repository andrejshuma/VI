...
def remove_column(dataset, min_idx):
    return [row[:min_idx] + row[min_idx+1:] for row in dataset]
...
if __name__ == '__main__':
    ...
    min_idx = feature_importances.index(min(feature_importances))

    train_X  = remove_column(train_X,min_idx)
    test_X  = remove_column(test_X,min_idx)

    neuron_classifier = MLPClassifier(15,activation="relu",learning_rate_init=0.001,max_iter=200,random_state=0)
    standard_scaler = StandardScaler()
    standard_scaler.fit(train_X)
    minmax_scaler = MinMaxScaler()
    minmax_scaler.fit(train_X)

    neuron_classifier.fit(standard_scaler.transform(train_X), train_Y)
    acc1 = neuron_classifier.score(standard_scaler.transform(test_X),test_Y)

    neuron_classifier.fit(minmax_scaler.transform(train_X), train_Y)
    acc2 = neuron_classifier.score(minmax_scaler.transform(test_X),test_Y)
