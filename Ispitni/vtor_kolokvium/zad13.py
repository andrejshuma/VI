...
def get_classes(dataset):
    ...
    set_2 = [row for row in dataset if row[-1] == 2]
    return set_0, set_1, set_2



def split2(dataset, x1, x2):
    #Дрвo на одлука со записите од податочното множество во рангот од x1% до x2% од секоја класа.
    set_0, set_1, set_2 = get_classes(dataset)
    train_set = set_0[int(len(set_0) * x1):int(len(set_0) * x2)] + set_1[int(len(set_1) * x1):int(len(set_1) * x2)] + set_2[int(len(set_2) * x1):int(len(set_2) * x2)]

    train_X2 = [row[:-1] for row in train_set]
    train_Y2 = [row[-1] for row in train_set]

    return train_X2, train_Y2


def split1_3(dataset,x):
    ...
    train_set = set_0[:int(len(set_0) * x)] + set_1[:int(len(set_1) * x)] + set_2[:int(len(set_2) * x)]
...
def get_test(dataset, x2):
    set_0, set_1, set_2 = get_classes(dataset)
#од x2% до 100% од секоја класа

    test_set = set_0[int(len(set_0) * x2):] + set_1[int(len(set_1) * x2):] + set_2[int(len(set_2) * x2):]
...
if __name__ == '__main__':
    ...
    train_X1, train_Y1 = split1_3(dataset,x1)
    train_X2, train_Y2 = split2(dataset,x1,x2)
    train_X3, train_Y3 = split1_3(dataset,x2)
    ...
    test_X, test_Y = get_test(dataset,x2)
    ...
    for true,pred1,pred2,pred3 in zip(test_Y, predictions1, predictions2, predictions3):
        counter = 0
        if true == pred1:
            counter += 1
        if true == pred2:
