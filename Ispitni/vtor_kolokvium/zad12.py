...
def remove_column(train_X, column_index):
    return [row[:column_index] + row[column_index+1:] for row in train_X]


def calculate_accuracy(predictions, true):
    acc=0

    for tr,pred in zip(true, predictions):
        if tr == pred:
            acc += 1
...
if __name__ == '__main__':
    ...
    entry_losh = input().split(" ")
    entry = []
    for el in entry_losh:
        entry.append(float(el))

    train_X,train_Y,val_X,val_Y = data_split(dataset)

    scaler  = MinMaxScaler(feature_range=(-1, 1))
    scaler.fit(train_X)
    classifier = MLPClassifier(hidden_layer_sizes=num_neurons, activation="relu",max_iter=20, learning_rate_init=learning_rate, random_state=0)
    classifier.fit(scaler.transform(train_X), train_Y)

    acc_train = calculate_accuracy(classifier.predict(scaler.transform(train_X)), train_Y)
    acc_val = calculate_accuracy(classifier.predict(scaler.transform(val_X)), val_Y)

    if acc_train > acc_val*1.15:
        print("Se sluchuva overfitting")
        train_X_new = remove_column(train_X, column_index)
        scaler.fit(train_X_new)
        classifier.fit(scaler.transform(train_X_new), train_Y)
        entry = entry[:column_index] + entry[column_index+1:]
    else:
