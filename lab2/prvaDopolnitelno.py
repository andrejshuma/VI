...
class Table(Problem):
    ...
    def successor(self, state):
        ...
        home_pos = state[1]
        home_dir = state[2]
        pravec = state[2] #promena

        ways_to_go = [(0, 0), (0, 3), (0, 2), (-1, 1), #promena
                      (-1, 2), (1, 1), (1, 2)]
        dirs_to_go = ["Stoj", "Gore 3", "Gore 2", #promena
        ...
        for way, dir in zip(ways_to_go, dirs_to_go):
            ...
            # promena
            new_pravec = dir.split(" ")[0]
            flag = False
            if new_pravec=="Stoj": flag=True
            elif new_pravec!=pravec: flag=True
            # promena


            if self.in_bounds(new_player_x,new_player_y) and flag: #promena
                new_player = (new_player_x, new_player_y)

                if new_player in self.allowed or new_player == new_home_pos:
                    new_state = (new_player, new_home_pos, new_home_dir,new_pravec) #promena
...
if __name__ == '__main__':
    ...
    initial_state = (person_coord, house_coord, house_dir,None) #promena
