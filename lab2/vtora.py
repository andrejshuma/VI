...
class Choveche(Problem):
    def __init__(self, initial, walls, n,kukja, goal=None):
    ...
    def check_valid(self,state,move):
        ...
        if move.endswith("2"):
            prethoden = (state[0]-1,state[1])
            if prethoden in self.walls: return False
        elif move.endswith("3"):
            prethoden = (state[0] - 1, state[1])
            pred_prethoden = (state[0] - 2, state[1])
            if prethoden in self.walls or pred_prethoden in self.walls: return False

        return 0<= state[0] <self.n and 0<= state[1] <self.n

    def successor(self, state):
        successors = dict()
        x,y = state

        moves = ["Desno 2","Desno 3","Gore","Dolu","Levo"]
        moves_coords = [(+2,0),(+3,0),(0,+1),(0,-1),(-1,0)]

        for move,coord in zip(moves,moves_coords):
            #print(move)
            #print(coord)
            new_human = (x+coord[0],y+coord[1])
            #print(new_human)
            if self.check_valid(new_human,move):
                successors[move]=tuple(new_human)

        return successors

    def h(self, node):
        x,y = node.state
        house_x, house_y = self.kukja
        return (abs(x-house_x) + abs(y-house_y)) /4
...
if __name__ == '__main__':
    ...
    for i in range(num_walls):
        walls.append(tuple([int(i) for i in input().split(",")]))

    choveche = tuple([int(i) for i in input().split(",")])
    kukja = tuple([int(i) for i in input().split(",")])

    problem = Choveche(tuple(choveche),walls,n,kukja)
